...
planet.z = np.zeros(T); planet.z[0] = planet.z0
...
#Definition des variables de milieux
	...
	vx_demi = planet.vx[i] + (dt/2)*fx(soleil.masse, planet.x[i], planet.y[i], planet.z[i])
	vy_demi = planet.vy[i] + (dt/2)*fy(soleil.masse, planet.x[i], planet.y[i], planet.z[i])
	vz_demi = planet.vz[i] + (dt/2)*fz(soleil.masse, planet.x[i], planet.y[i], planet.z[i])
...
# Affectation des positions à l'indice i+1
	...
	planet.vx[i+1] = vx_demi + (dt/2)*fx(soleil.masse, planet.x[i+1], planet.y[i+1], planet.z[i+1])
	planet.vy[i+1] = vy_demi + (dt/2)*fy(soleil.masse, planet.x[i+1], planet.y[i+1], planet.z[i+1])
...
Nrg = E(soleil.masse, planet.x, planet.y, planet.z, planet.vx, planet.vy, planet.vz)
