...
#Definition des variables de milieux
	...
	x_demi = sonde.x[i] + (ds/(2*rho1))*sonde.vx[i];  sonde.x[i] = x_demi
	y_demi = sonde.y[i] + (ds/(2*rho1))*sonde.vy[i];  sonde.y[i] = y_demi
	z_demi = sonde.z[i] + (ds/(2*rho1))*sonde.vz[i];  sonde.z[i] = z_demi
...
#Define Rho 2 -> rho[i+1]
	...
	# Affectation des positions à l'indice i+1
	sonde.vx = np.append(sonde.vx , vx_demi + (ds/(2*rho2))*fx)
	sonde.vy = np.append(sonde.vy , vy_demi + (ds/(2*rho2))*fy)
	sonde.vz = np.append(sonde.vz , vz_demi + (ds/(2*rho2))*fz)
...
#Traj Horizon reelle
x, y, z = np.genfromtxt("horizons_results.txt", usecols=(4,5,6), unpack=True) 
